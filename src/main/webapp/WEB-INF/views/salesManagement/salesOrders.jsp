				...
				<div class="row">
					<div class="col-lg-12">
						...
						<ul id="myTab" class="nav nav-tabs">
							<li class="active"><a href="#Unconfirmed" data-toggle="tab">待确认订单</a></li>
							<li><a href="#underway" data-toggle="tab">进行中订单</a></li>
							<li><a href="#finish" data-toggle="tab">已完成订单</a></li>
						</ul>

						<div id="myTabContent" class="tab-content">
							<div class="tab-pane fade in active" id="Unconfirmed">
								<br>
								<c:if test="${!empty listOrder0}">
									<table class="table table-hover">
										...
										<c:forEach items="${listOrder0}" var="ord">
											<tr>
												<td>${ord.sproduct.name}</td>
												<td>${ord.cost}</td>
												<td>${ord.count}</td>
												<td>${ord.adress }</td>
												<td>${ord.tel }</td>
												<td><a class="btn btn-outline btn-info btn-sm"
													href="<c:url value='/SM-ord/comfirm/${ord.id}' />">确认</a></td>
											</tr>
										</c:forEach>
									</table>
								</c:if>
							...
							<div class="tab-pane fade" id="underway">
								<br>
									...
									<table class="table table-hover">
										...
										<c:forEach items="${listOrder12}" var="ord">
											<tr>
												...
												<td>${ord.adress }</td>
												<td>${ord.tel }</td>
												<td><c:if test="${ord.status ==1}">待发货</c:if> <c:if
														test="${ord.status ==2}">配送中</c:if></td>
											</tr>
										</c:forEach>
									</table>
								</c:if>
